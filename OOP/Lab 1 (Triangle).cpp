...
public:
	Triangle() : side1(1), side2(1), side3(1), sideType(None), angleType(None) {};
	Triangle(const float a, const float b, const float c) : side1(a), side2(b), side3(c), sideType(FormSides()), angleType(FormAngles()) {
		if (!Exist(a, b, c)) side1 = 1, side2 = 1, side3 = 1, sideType = None, angleType = None;
	};
...
void Triangle::Read() {
	...
	do {
		...
		b = Check();
		c = Check();
		if (!Exist(a, b, c)) cout << "\nНеправильные стороны! Введите повторно: ";
	} while (!Exist(a, b, c));
