...
string Time::toString() const noexcept {
    string space = "";
    return "\n" + (hour.GetNum() > 9 ? space : "0" ) + to_string(hour.GetNum())
...
unsigned int Time::ConvertInSeconds() const noexcept {
    return hour.GetNum() * 3600 + minute.GetNum() * 60 + second.GetNum();
}
unsigned short Time::ConvertInMinutes() const noexcept {
    return static_cast<unsigned short>(round(static_cast<float>(ConvertInSeconds()) / 60));
}
//==============================================================================================
